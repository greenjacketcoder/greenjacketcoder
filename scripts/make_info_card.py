...
ROWS = [
    ...
    ("gap",),
    ("sec", "Stack"),
    ("kv", "Languages", "PowerShell, Python, Bash"),
    ("kv", "Platforms", "Windows, macOS"),
    ("kv", "CI / CD", "GitHub Actions, Pester"),
    ("gap",),
    ("sec", "Projects"),
    ("bul", "hardeningtomcat — Windows audit & hardening engine"),
    ("bul", "reveille — macOS menu-bar meeting alerts"),
    ("bul", "Swift-Bar-Stock-Ticker — macOS menu-bar ticker"),
]
...
parts.append(f'<text x="{W/2}" y="{TITLEBAR_H/2 + 4}" fill="{MUTED}" font-size="12" '
             f'text-anchor="middle">greenjacketcoder@github: ~$ neofetch</text>')
...
for i, row in enumerate(ROWS):
    ...
    if kind == "host":
        inner = (f'<text x="{KEY_X}" y="{y:.1f}" font-size="14" font-weight="700">'
                 f'<tspan fill="{GREEN}">greenjacketcoder</tspan><tspan fill="{MUTED}">@</tspan>'
                 f'<tspan fill="{ACCENT}">github</tspan></text>'
                 f'<line x1="{KEY_X+206}" y1="{y-4:.1f}" x2="{W-PAD}" y2="{y-4:.1f}" '
